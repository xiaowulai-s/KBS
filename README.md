...
一个基于 GitHub Pages 部署的静态 PWA 应用，用于将本地文件整理成结构化的在线知识库。支持本地后端服务持久化、批量管理、回收站、全文搜索和 Markdown 在线编辑。

## 功能特性

### 核心能力
- **知识库展示** - 列表/网格双视图，Industrial Editorial 深色/浅色主题
- **全文搜索** - 基于 MiniSearch 的前端全文检索，支持标题、描述、标签、分类搜索
- **分类与标签** - 树形分类、标签云、筛选面包屑，支持元数据重命名/合并/清理
- **Markdown 预览与编辑** - 点击卡片预览 Markdown，内置实时预览编辑器
- **附件上传** - 拖拽上传文件到 `docs/` 目录（需启动本地后端服务）

### 管理增强
- **管理面板** - 增删改查索引条目、分类统计仪表盘、操作历史
- **批量操作** - 批量移动分类、打标签、收藏、删除、恢复
- **回收站** - 软删除、恢复、永久删除，避免误删
- **数据导入导出** - 支持 JSON、CSV、Markdown 压缩包
